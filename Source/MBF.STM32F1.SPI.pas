...
type
  ...
  protected
    ...
    procedure SetOperatingMode(const {%H-}aOperatingMode: TSPIOperatingMode);
  public
...
begin
  ...
  ValidConfig := True;
  {$if defined(has_SPI1)}
  if longInt(ASCLKPin) and $ff = TNativePin.PA5 then
  begin
    if not (((longInt(AMosiPin) and $ff) = TNativePin.None) or ((longInt(AMosiPin) and $ff) = TNativePin.PA7)) then
      ValidConfig := false;
    if not (((longInt(AMisoPin) and $ff) = TNativePin.None) or ((longInt(AMisoPin) and $ff) = TNativePin.PA6)) then
      ValidConfig := false;
    //if not (((longInt(ANssPin and $ff) <> TNativePin.None) or ((ANssPin and $ff) <> TNativePin.PA4)) then
    //  ValidConfig := false;
  end;
  if longInt(ASCLKPin) and $ff = TNativePin.PB3 then
  begin
    if not (((longInt(AMosiPin) and $ff) = TNativePin.None) or ((longInt(AMosiPin) and $ff) = TNativePin.PB5)) then
      ValidConfig := false;
    if not (((longInt(AMisoPin) and $ff) = TNativePin.None) or ((longInt(AMisoPin) and $ff) = TNativePin.PB4)) then
      ValidConfig := false;
    //if not (((longInt(ANssPin and $ff) = TNativePin.None) or ((longInt(ANssPin) and $ff) = TNativePin.PA15)) then
    //  ValidConfig := false;
  end;
  {$endif}
  {$if defined(has_SPI2)}
  if longInt(ASCLKPin) and $ff = TNativePin.PB13 then
  begin
    if not (((longInt(AMosiPin) and $ff) = TNativePin.None) or ((longInt(AMosiPin) and $ff) = TNativePin.PB15)) then
      ValidConfig := false;
    if not (((longInt(AMisoPin) and $ff) = TNativePin.None) or ((longInt(AMisoPin) and $ff) = TNativePin.PB14)) then
      ValidConfig := false;
    //if not (((longInt(ANssPin and $ff) = TNativePin.None) or ((longInt(ANssPin) and $ff) = TNativePin.PB12)) then
    //  ValidConfig := false;
  end;
  {$endif}
  {$if defined(has_SPI1)}
  if longInt(ASCLKPin) and $ff = TNativePin.PA15 then
  begin
    if not (((longInt(AMosiPin) and $ff) = TNativePin.None) or ((longInt(AMosiPin) and $ff) = TNativePin.PB5)) then
      ValidConfig := false;
    if not (((longInt(AMisoPin) and $ff) = TNativePin.None) or ((longInt(AMisoPin) and $ff) = TNativePin.PB4)) then
      ValidConfig := false;
    //if (((longInt(ANssPin and $ff = TNativePin.None) or ((longInt(ANssPin and $ff) = TNativePin.PA15)) then
    //  ValidConfig := false;
  end;
  if longInt(ASCLKPin) and $ff = TNativePin.PC10 then
  begin
    if not (((longInt(AMosiPin) and $ff) = TNativePin.None) or ((longInt(AMosiPin) and $ff) = TNativePin.PC12)) then
      ValidConfig := false;
    if not (((longInt(AMisoPin) and $ff) = TNativePin.None) or ((longInt(AMisoPin) and $ff) = TNativePin.PC11)) then
      ValidConfig := false;
    //if not (((longInt(ANssPin) and $ff) = TNativePin.None) or ((longInt(ANssPin) and $ff) = TNativePin.PA4)) then
    //  ValidConfig := false;
  end;
  {$endif}

  if ValidConfig = false then
    asm
    .Lloop:
     bkpt
    b .Lloop
  ...
  for i := 1 to SPICount do
    NSSPins[i] := longInt(TNativePin.None);
  ...
  case {%H-}longWord(@Self) of
      {$ifdef has_spi1}SPI1_BASE : begin
        ...
        if (longWord(ASCLKPIN) and $ff = longWord(TNativePin.PA5)) then
      ...
      {$ifdef has_spi3}SPI3_BASE : begin
        ...
        if (longWord(ASCLKPIN) and $ff = longWord(TNativePin.PB3)) then
