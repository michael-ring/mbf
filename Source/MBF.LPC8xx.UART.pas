...
{$ENDREGION}
  ...
  public
    ...
    { Reads string from UART (serial) port.
      ...
      @returns(Number of bytes that were actually read.) }
    function ReadString(out Text: StdString; const MaxCharacters: longWord = 0;
    ...
    { Writes string to UART (serial) port.
        ...
        @returns(Number of bytes that were actually read.) }
    function WriteString(const Text: StdString; const TimeOut: TMilliSeconds = 0): Boolean;
...
function TUARTRegistersHelper.ReadString(out Text: StdString; const MaxCharacters: longWord = 0;
  const TimeOut: TMilliSeconds = 0): Boolean;
begin
  Text := '';
  Result := false;
...
function TUARTRegistersHelper.WriteString(const Text: StdString; const TimeOut: TMilliSeconds = 0): Boolean;
begin
