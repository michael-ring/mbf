...
begin
  ...
  SPI.TransferWord($0900); //Do not decode bits
  SPI.TransferWord($0a05); //Brightness of LED's
  SPI.TransferWord($0b07); //Show all scan lines
  SPI.TransferWord($0c01); //Display on
  SPI.TransferWord($0f00); //TestMode off

  repeat
    //Clear Display
    for i := 1 to 8 do
      SPI.TransferWord((i shl 8) + 0);
    SystemCore.Delay(1000);
    for i := 1 to 8 do
      SPI.TransferWord((i shl 8) + $ff);
    SystemCore.Delay(1000);
    SPI.Write(@PatternA,SizeOf(PatternA));
    SystemCore.Delay(1000);
  until 1=0
end.
